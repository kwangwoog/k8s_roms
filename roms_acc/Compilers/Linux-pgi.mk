...
#               FC := tau_f90.sh
           FFLAGS := 
#           FFLAGS := -acc
#           FFLAGS := -acc -Minfo -ta=tesla:cc35  -fast -O3 -Mvect=fuse
#           FFLAGS := -acc -Minfo -ta=multicore -fast -O3 -Mvect=fuse
              CPP := /usr/bin/cpp
...
ifdef USE_NETCDF4
#        NC_CONFIG ?= nc-config
#        NC_CONFIG ?= /usr/local/netcdf/432_pgi1510
#    NETCDF_INCDIR ?= /usr/local/netcdf/432_pgi1510/include
    NETCDF_INCDIR ?= /usr/local/netcdf/pgi/include
#             LIBS := -L/usr/local/netcdf/432_pgi1510/lib -lnetcdff -lnetcdf
             LIBS := -L/usr/local/netcdf/pgi/lib -lnetcdff -lnetcdf
else

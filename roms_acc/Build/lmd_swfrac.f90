     ...
     &                            Zscale, zgrid, swdk)
!svn $Id: lmd_swfrac.F 751 2015-01-07 22:56:36Z arango $
!================================================== Hernan G. Arango ===
...
!      real(r8), intent(in) :: Z(IminS:ImaxS,JminS:JmaxS)
      real(r8), intent(in) :: zgrid(IminS:ImaxS,JminS:JmaxS)
...
!  absorption model.
!-----------------------------------------------------------------------
!$acc enter data copyin(mixing(ng))
!$acc enter data copyin(MIXING(ng)%Jwtype)
!$acc data create(fac1,fac2,fac3) copyin(zgrid,lmd_mu1,lmd_mu2,lmd_r1) copyout(swdk)
!$acc kernels loop independent present(mixing(ng)) private(Jindex)
      DO j=Jstr,Jend
        DO i=Istr,Iend
        ...
        END DO
        DO i=Istr,Iend
          swdk(i,j)=EXP(zgrid(i,j)*fac1(i))*fac3(i)+                    &
     &              EXP(zgrid(i,j)*fac2(i))*(1.0_r8-fac3(i))
        END DO
      END DO
!$acc end kernels
!$acc exit data
